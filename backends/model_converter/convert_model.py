...
def convert_model(checkpoint_filename=None, out_filename=None,  torch_weights=None):
    ...
    for k in (list(state_dict.keys())):
        ...
        if ".ff." in k:
            pp = state_dict[k]
            state_dict[f"{k}._split_1"] = pp[:pp.shape[0]//2].copy()
            state_dict[f"{k}._split_2"] = pp[pp.shape[0]//2:].copy() 

        elif "attn.in_proj" in k and  state_dict[k].shape[0] == 3072:
            pp = state_dict[k]
            state_dict[f"{k}._split_1"] = pp[:pp.shape[0]//3].copy()
            state_dict[f"{k}._split_2"] = pp[pp.shape[0]//3:2*pp.shape[0]//3].copy()
            state_dict[f"{k}._split_3"] = pp[2*pp.shape[0]//3: ].copy()
    ...
    for i in range(1,21):
        ...
        state_dict[f"zeros_{str(nn)}"] = np.zeros(nn).astype(dtype)
        state_dict[f"ones_{str(nn)}"] = np.ones(nn).astype(dtype)
